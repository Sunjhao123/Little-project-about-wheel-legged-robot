...
class NoiseMapper:
    def __init__(self):
        ...
        # 噪音参数
        self.min_db = 30.0          # 最小显示分贝
        self.max_db = 100.0         # 最大显示分贝
        self.decay_rate = 0.95      # 噪音衰减率
        ...
        try:
            ...
            for dx in range(-effect_radius, effect_radius+1):
                for dy in range(-effect_radius, effect_radius+1):
                    ...
                    decay = max(0.0, 1.0 - distance/effect_radius)
                    value = msg.decibel * decay
                    
                    col = grid_col + dx
                    row = grid_row + dy
                    if 0 <= col < self.grid_dim and 0 <= row < self.grid_dim:
                        self.noise_grid[row, col] = max(value, self.noise_grid[row, col])
