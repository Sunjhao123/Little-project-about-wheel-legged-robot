...
namespace BAstar_planner
{
  ...
  {    
    ...
    distance1 = sqrt(pow(x1,x2)+pow(y1,y2));       //Euclidean Distance
    h = (min(abs(x1-x2),abs(y1-y2))  *sqrt(2)  + max(abs(x1-x2),abs(y1-y2)) - min(abs(x1-x2),abs(y1-y2)))*0.6+distance1*0.4;   //Diagonal Distance
  ...
  /*(float calculateWeight(float h_cost)
  {
  ...
  void BAstarPlannerROS::addNeighborCellToOpenList(multiset<cells> &OPL, int neighborCell, int goalCell, float g_score[])
  {
    ...
    h_cost = calculateHCost(neighborCell, goalCell);

    if(h_cost>30)
    {
      wei = 2.0;
    }
    else if(h_cost<=30&&h_cost>10)
    {
      wei = 1.0;
    }
    else{
    ...
    CP.fCost = g_score[neighborCell] + h_cost*wei;
    OPL.insert(CP);
